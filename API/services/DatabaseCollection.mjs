...
const initialiseDatabaseSearch = (searchParams) => {
    ...
    if (databaseNames.includes(searchParams)){
        ...
        // check for partial match on database and collection names

        for (let i =0; i<= databaseNames.length-1; i++){
            for (let collection in totalFieldValues[databaseNames[i]]){
                console.log(collection)
                if (Object.keys(totalFieldValues[databaseNames[i]][collection]).includes(searchParams)){
                    results.push({
                        Match:"Exact",
                        Type:"Field",
                        Field: searchParams,
                        AllFieldValues:Object.values(totalFieldValues[databaseNames[i]][collection][searchParams]),
                        Collection: collection,
                        Database:databaseNames[i],
                        AllFieldsURL:"api/v2/"+databaseNames[i]+"/"+collection+"/"+searchParams,
                        AllDocsUrl:"api/v2/"+databaseNames[i]+"/"+collection
                    })
                }
                else if (collection.match(partialExpression)){
                    //get exact field
                    results.push({
                        Match:"Partial",
                        Type:"Collection",
                        ...
                        URL:"api/v2/"+databaseNames[i]+"/"+collection
                    })
                }
                else {
                    //check partial match on fields
                    const fieldString =  Object.keys(totalFieldValues[databaseNames[i]][collection]).toString();
                    
                    const matched = fieldString.match(partialExpression)
                    if (matched){
                        //get exact field
                        ...
                        results.push({
                            ...
                            Type:"Field",
                            Field: "s",
                            AllFieldValues:Object.values(totalFieldValues[databaseNames[i]][collection][matched.toString()]),
                            Collection: collection,
                            Database:databaseNames[i],
                            AllFieldsURL:"api/v2/"+databaseNames[i]+"/"+collection+"/"+searchParams,
                            AllDocsUrl:"api/v2/"+databaseNames[i]+"/"+collection
            ...
            } // for collection maps in totalfield values
        } // looping through dbs
    }
